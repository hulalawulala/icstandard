...
document.getElementById('btnDownloadPNGs').addEventListener('click', () => {
    ...
    if (appState.front.hasResult) {
        // Use clean offscreen canvas so corners are white, not transparent/black
        const frontData = getCleanCanvasDataURL('canvasOutput-front');
        if (frontData) {
            const link = document.createElement('a');
            link.download = 'cropped-ic-front.jpg';
            link.href = frontData;
            link.click();
        }
    }
    ...
    setTimeout(() => {
        if (appState.back.hasResult) {
            // Use clean offscreen canvas so corners are white, not transparent/black
            const backData = getCleanCanvasDataURL('canvasOutput-back');
            if (backData) {
                const link = document.createElement('a');
                link.download = 'cropped-ic-back.jpg';
                link.href = backData;
                link.click();
            }
        }
...
/**
 * Renders the processed IC canvas onto a clean white offscreen canvas
 * so that JPEG export does not produce blurry/grey artefacts from the
 * transparent rounded-corner clipping mask.
 */
function getCleanCanvasDataURL(canvasId) {
    const src = document.getElementById(canvasId);
    if (!src || src.width === 0 || src.height === 0) return null;

    // Use a high-resolution multiplier so the PDF looks sharp when printed
    const SCALE = 3;
    ...
    // Draw the rounded-corner IC shape as a clip path
    const radius = offscreen.width * 0.037;
    ctx.beginPath();
    ctx.moveTo(radius, 0);
    ctx.lineTo(offscreen.width - radius, 0);
    ctx.quadraticCurveTo(offscreen.width, 0, offscreen.width, radius);
    ctx.lineTo(offscreen.width, offscreen.height - radius);
    ctx.quadraticCurveTo(offscreen.width, offscreen.height, offscreen.width - radius, offscreen.height);
    ctx.lineTo(radius, offscreen.height);
    ctx.quadraticCurveTo(0, offscreen.height, 0, offscreen.height - radius);
    ctx.lineTo(0, radius);
    ctx.quadraticCurveTo(0, 0, radius, 0);
    ctx.closePath();
    ctx.clip();

    // Paint the source IC image into the clipped area (scaled up for sharpness)
...
document.getElementById('btnExportPDF').addEventListener('click', () => {
    ...
    // Malaysian IC standard size: 86mm x 54mm
    const icWidth = 86;
    const icHeight = 54;
    ...
    if (appState.front.hasResult) {
        const frontData = getCleanCanvasDataURL('canvasOutput-front');
        if (frontData) doc.addImage(frontData, 'JPEG', xPos, 50, icWidth, icHeight);
    }

    if (appState.back.hasResult) {
        const backData = getCleanCanvasDataURL('canvasOutput-back');
        if (backData) doc.addImage(backData, 'JPEG', xPos, 150, icWidth, icHeight);
    }
